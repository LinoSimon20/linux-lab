...
| Frecuencia | Mensaje |
|------------|---------|
$(grep -E '\| (ERROR|FATAL) \|' "$LOGFILE" | cut -d '|' -f4 | sed 's/^ *//' | sort | uniq -c | sort -rn | head -5 | awk '{ count=$1; $1=""; sub(/^ +/,"",$0); printf "| %d | %s |\n", count, $0 }')

## 5. Resumen

- Sistema analizado con $(wc -l < "$LOGFILE") eventos registrados  
- $(grep -cE '\| (ERROR|FATAL) \|' "$LOGFILE" 2>/dev/null || echo 0) eventos requieren atención (ERROR y FATAL)  
